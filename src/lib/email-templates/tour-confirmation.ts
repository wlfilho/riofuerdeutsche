...
function parseTourDetails(text: string): string {
  ...
  return text
    ...
    .map(line => `<div style="padding: 3px 0;">${line}</div>`)
    .join('')
}
...
export function tourConfirmationEmail(client: TourClient) {
  ...
  const tourDetailsBlock = client.tour_details
    ? `<tr>
        <td style="padding:4px 0;color:#555;font-size:14px;vertical-align:top;"><strong>🗺️ Tour:</strong></td>
        <td style="padding:4px 0 4px 16px;color:#333;font-size:14px;">${parseTourDetails(client.tour_details)}</td>
      </tr>`
    : ''

  const restbetrag = (client.total_amount ?? 0) - (client.deposit_amount ?? 0)
              ...
              <tr>
                <td>
                  ...
                  <table cellpadding="0" cellspacing="0">
                    <tr>
                      <td style="padding:4px 0;color:#555;font-size:14px;"><strong>✈️ Ankunft:</strong></td>
