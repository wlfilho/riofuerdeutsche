...
const CHANNELS = [
  { von: 'whatsapp', label: 'WhatsApp', icon: '💬' },
  { von: 'email', label: 'E-Mail', icon: '✉️' },
  { von: 'instagram', label: 'Instagram', icon: '📷' },
] as const;
...
export default function AnfrageLinkButton() {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        {copied ? '✓ Link copiado' : '🔗 Neue Anfrage'}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1">
          <p className="px-3 py-1.5 text-xs text-gray-400">Copiar link para enviar via:</p>
